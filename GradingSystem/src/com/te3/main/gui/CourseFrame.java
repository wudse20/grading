		...
		try {
			...
			for (var i = 0; i < al2.size(); i++) {
				for (var j = 0; j < al2.get(i).getStudents().size(); j++) {
					Course c = new Course(np.getLastInput(), mf.getMainData().getClasses().get(mf.getCurrentlySelectedClassIndex()).getName(), mcp.getCriteria());
					...
					for (int j = 0; j < allSchoolClasses.get(i).getStudents().size(); j++) {
						...
						if (orgSchoolClasses.contains(sc)) {
							...
							allSchoolClasses.get(i).getStudents().get(j)
									.addCourse(new Course(np.getLastInput(), mf.getMainData().getClasses().get(mf.getCurrentlySelectedClassIndex()).getName(), mcp.getCriteria(), c.getCourseTasks()));
							...
							allSchoolClasses.get(i).getStudents().get(j)
									.addCourse(new Course(np.getLastInput(), mf.getMainData().getClasses().get(mf.getCurrentlySelectedClassIndex()).getName(), mcp.getCriteria(), c.getNewCourseTask()));

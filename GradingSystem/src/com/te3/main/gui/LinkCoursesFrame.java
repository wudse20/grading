...
public class LinkCoursesFrame extends JFrame implements WindowListener {
	...
	private String helpText = "För att länka en kurs till klassen<br>"
			...
			+ "För att avlänka en kurs klicka på kursen i listan: <br>" + "<b>Tillagda Kurser</b>. <br><br>"
			+ "För att applicera ändringarna tryck på: <b>Verkställ</b> <br><br>"
			+ "För att avbryta tryck antingen på: <b>Avbryt</b><br>" + "Eller stäng fönstret.";
	 ...
	 * @param addedCourses the already linked courses, {@code null} if no courses
	 *                     added.
	 */
	public LinkCoursesFrame(MainSchoolClassPanel mscp, ArrayList<Course> addedCourses, MainFrame mf) {
		...
		btnHelp.addActionListener(e -> new HelpFrame("Länka kurser", "<html><p>" + helpText + "</p></html>",
	...
	private void btnApplyPressed() {
		// Handles errors
		// If there are no courses added, then debug message, message to user and it
		// skips the rest of the method.
		if (addedCourses.size() == 0) {
			...
			System.out.println("["
					...
					+ "] LinkCoursesFrame: No added courses, skipping rest of the operations...");

			// Message to user
			JOptionPane.showMessageDialog(this,
					"Du har inga kurser valda; vill du länka kurser, öppna rutan igen och välj dem!",
					"Inga valda kurser!", JOptionPane.INFORMATION_MESSAGE);
		...
		for (int i = 0; i < addedCourses.size(); i++) {
			try {
				...
				courses.add(new Course(addedCourses.get(i).getName(),
						mf.getMainData().getClasses().get(mf.getCurrentlySelectedClassIndex()).getName(),
						addedCourses.get(i).getCourseCriteria(), addedCourses.get(i).getNewCourseTask()));
				...
				System.out.println("["
						...
						+ ((LocalTime.now().getSecond() < 10) ? "0" + LocalTime.now().getSecond()
								: LocalTime.now().getSecond())
						+ "] LinkCoursesFrame: Added a blank Course to the list with name: "
		...
		// Message to user
		JOptionPane.showMessageDialog(this, "Du har uppdaterat klassens kurser!", "Uppdatering",
		...
		System.out.println("["
				...
				+ "] LinkCoursesFrame: has updated MainSchoolClassPanel");
	...
	private void grabDataFromMainFrame() {
		...
		for (var i = 0; i < classes.size(); i++) {
			...
			for (var j = 0; j < courses.size(); j++) {
				...
				Course c = null;
				try {
					c = new Course(courses.get(j).getName(),
							mf.getMainData().getClasses().get(mf.getCurrentlySelectedClassIndex()).getName(),
							courses.get(j).getCourseCriteria(), courses.get(j).getCourseTasks());
				} catch (IllegalNameException e) {
					e.printStackTrace();
				}
				...
				if (!this.addedCourses.contains(c)) {
					...
					System.out.println("["
							...
							+ ((LocalTime.now().getSecond() < 10) ? "0" + LocalTime.now().getSecond()
									: LocalTime.now().getSecond())
							+ "] LinkedCoursesFrame: Added course: " + c.getName() + ", to not added courses.");
				} else {
					// Debug message:
					System.out.println("["
							+ ((LocalTime.now().getHour() < 10) ? "0" + LocalTime.now().getHour()
							...
							+ ((LocalTime.now().getSecond() < 10) ? "0" + LocalTime.now().getSecond()
									: LocalTime.now().getSecond())
							+ "] LinkedCoursesFrame: " + c.getName() + " is in the added courses list.");
				}
			}
		}
	}
	...
	private void btnCancelPressed() {
		...
		int ans = JOptionPane.showConfirmDialog(this, "Är du säker på att du vill avbryta?", "Avbryt",

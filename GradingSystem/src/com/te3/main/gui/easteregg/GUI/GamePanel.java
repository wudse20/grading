...
public class GamePanel extends JPanel implements KeyListener, MouseListener, MouseMotionListener {
	/** Generated */
	private static final long serialVersionUID = 2835997072209492263L;
	...
	private boolean round(Move pMove) {
		...
		/*
		 * While there moves are the same the computer will try again until they differ,
		 * since draws are for losers according to this program.
		 */
		do {
	...
	private void drawButtons(Graphics2D g) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (i == 0) {
				...
				// Draws the scissors
				this.drawScissors(g, x + btnPadding, y + btnPadding, squareSideLength - (2 * btnPadding),
						squareSideLength - (2 * btnPadding));
			} else {
				// Draws the stone
				this.drawPaper(g, x + btnPadding, y + btnPadding, squareSideLength - (2 * btnPadding),
						squareSideLength - (2 * btnPadding));
			}
		}
	}
	...
	/**
	 * Check if a button is clicked and acts on it.
	 *
	...
	private void detectButtonPress(int x, int y) {
		/*
		 * Checks which button is pressed on mouse click wont do anything if it's
		 * pressed outside of the buttons. Will only do it if the game is running at the
		 * moment.
		 *
		 * It will also act on the click and play a round.
		 */
		if (((x > space && x < space + squareSideLength) && (y > space && y < space + squareSideLength))
				&& isGameRunning) {
			if (round(Move.ROCK)) {
				pWins++;
			} else {
			...
			this.repaint();
		} else if (((x > space * 2 + squareSideLength && x < space * 2 + squareSideLength * 2)
				&& (y > space && y < space + squareSideLength)) && isGameRunning) {
			...
			this.repaint();
		} else if (((x > space * 3 + squareSideLength * 2 && x < space * 3 + squareSideLength * 3)
				&& (y > space && y < space + squareSideLength)) && isGameRunning) {
			if (round(Move.PAPER)) {
				pWins++;
			} else {
	...
	private void hoverEffect(int x, int y) {
		/*
		 * Checks if the mouse i hovering over a button. If it is then it will tell the
		 * program that and then repaint. Will only do it if the game is running at the
		 * moment.
		 */
		if (((x > space && x < space + squareSideLength) && (y > space && y < space + squareSideLength))
				&& isGameRunning) {
	...
	public void paintComponent(Graphics gOld) {
		...
		/*
		 * If the game should run it runs it, else it will show the start message.
		 */
		if (!isGameRunning) {

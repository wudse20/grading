...
public class Task {
	 ...
	 * {@link #getCriteria()} () Use this method for: normal getter for the
	 * criteria}
	 *
		...
		for (int i = 0; i < this.criteria.size(); i++) {
			try {
				...
				System.out.println("["
						...
						+ "] Task: Created a new criteria with the name: " + this.criteria.get(i).getName());
				...
				System.out.println("["
						...
						+ ":" + ((LocalTime.now().getSecond() < 10) ? "0" + LocalTime.now().getSecond()
								: LocalTime.now().getSecond())
						+ "] Task: Exception thrown: " + e.getMessage());
			}
		}
